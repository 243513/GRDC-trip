...
def run_training_phase(model, optimizer, loader, phase, device):
    ...
    param_groups = [
        ...
        {"params": contrast_params, "lr": LEARNING_RATE * 10}  
    ]
    ...
    with tqdm(loader, desc=phase_desc, unit="batch") as pbar:
        for batch in pbar:
            ...
            if phase == "pretrain":
                ...
                loss_components['contrast_loss'] += contrast_loss.item() if isinstance(contrast_loss,                                                                   torch.Tensor) else contrast_loss
            elif phase == "finetune":
                gen_loss = outputs['gen_loss']
                loss_components['gen_loss'] += gen_loss.item() if isinstance(gen_loss, torch.Tensor) else gen_loss
            scaled_loss = loss * 5.0  
            optimizer.zero_grad()
...
def evaluate_model(model, evaluator, df, phase_name, num_samples=100):
    ...
    geo_score = results.get('avg_geo_score', 0.0) 
